...
def parse_text(text, cmudict):
    ...
    # check for words not in pronunciation dictionary
...
def format_alt_query(text):
    return alt_entry_pattern.sub(r'{\1}', text)

def load_cmudict():
    """Loads CMU Pronouncing Dictionary"""

    with open('./tacotron/cmudict-0.7b.txt', encoding='ISO-8859-1') as file:
        cmudict = (line.strip().split("  ") for line in islice(file, 126, 133905))
        cmudict = {
            format_alt_query(word): pronunciation for word, pronunciation in cmudict
        }
    return cmudict
